...
const CONTRACT_ABI = [
  "function registerComplaint(string memory _complaintId, string memory _complaintHash, string memory _beforeLandmarkHash) external",
  "function updateComplaintStatus(string memory _complaintId, uint8 _status) external",
  "function resolveComplaint(string memory _complaintId, string memory _resolutionHash, string memory _afterLandmarkHash) external",
  "function verifyComplaint(string memory _complaintId, string memory _complaintHash) external view returns (bool)",
  "function getComplaint(string memory _complaintId) external view returns (string memory, string memory, string memory, uint256, uint8, address)",
...
const registerComplaintOnChain = async (complaintId, complaintData) => {
  ...
  const landmarkHash = generateHash({ location: complaintData.location, timestamp: complaintData.timestamp });
  const contract = getContract();

  const tx = await contract.registerComplaint(complaintId, hash, landmarkHash);
...
const resolveComplaintOnChain = async (complaintId, resolutionData) => {
  ...
  const afterLandmarkHash = generateHash({ resolvedAt: resolutionData.resolvedAt, resolvedBy: resolutionData.resolvedBy });
  const contract = getContract();

  const tx = await contract.resolveComplaint(complaintId, resolutionHash, afterLandmarkHash);
